<?php encabezado() ?>
<!-- Begin Page Content -->
<div id="body" class="page-content bg-light pt-120">
    <section class="contenedor">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 mt-2">
                    <div class="row">
                        <div class="col-lg-6 mb-2">
                            <a class="btn btn-primary" href="<?php echo base_url(); ?>Ubigeo/Listar"><i class="fas fa-arrow-alt-circle-left"></i> Regresar</a>
                        </div>
                    </div>
                    <div class="text-center">
                        <h2> TABLA UBIGEOS INACTIVOS </h2>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="Table">
                            <thead id="blanco">
                                <tr>
                                    <th>Id</th>
                                    <th>Dirección</th>
                                    <th>Manzana / Lote</th>
                                    <th>Número</th>
                                    <th>Piso</th>
                                    <th>Referencia</th>
                                    <th>Código Postal</th>
                                    <th>Distrito</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($data as $ubi) { ?>
                                    <tr id="negro">
                                        <td><?php echo $ubi['id_ubigeo']; ?></td>
                                        <td><?php echo $ubi['direc_ubigeo']; ?></td>
                                        <td><?php echo $ubi['mzlote_ubigeo']; ?></td>
                                        <td><?php echo $ubi['num_ubigeo']; ?></td>
                                        <td><?php echo $ubi['piso_ubigeo']; ?></td>
                                        <td><?php echo $ubi['referen_ubigeo']; ?></td>
                                        <td><?php echo $ubi['cpostal_ubigeo']; ?></td>
                                        <td><?php echo $ubi['id_distrito']; ?></td>
                                        <td>
                                            <form action="<?php echo base_url() ?>Ubigeo/reingresar?id=<?php echo $ubi['id_ubigeo']; ?>" method="post" class="d-inline confirmar">
                                                <button type="submit" class="btn btn-success"><i class="fas fa-ad"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>
<?php pie() ?>